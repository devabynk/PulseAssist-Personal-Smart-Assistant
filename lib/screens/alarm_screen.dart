...
class _AlarmScreenState extends State<AlarmScreen> {
  ...
  Future<void> _editAlarm(Alarm alarm) async {
    await _showAddEditSheet(alarm: alarm);
  }
      ...
      if (result != null) {
        ...
        try {
          ...
          if (duplicate != null && mounted) {
            ...
            final timeStr = '${result.time.hour.toString().padLeft(2, '0')}:${result.time.minute.toString().padLeft(2, '0')}';
            String daysStr;
            
            if (result.repeatDays.isEmpty) {
              // One-time alarm - show date
              final now = DateTime.now();
              final isToday = result.time.day == now.day && result.time.month == now.month;
              final isTomorrow = result.time.day == now.add(const Duration(days: 1)).day;
              ...
              if (isToday) {
                ...
                daysStr = '${result.time.day}/${result.time.month}/${result.time.year}';
              }
            } else {
            ...
            builder: (ctx) => AlertDialog(
                ...
                actions: [
                    TextButton(
                        ...
                        child: Text(isTurkish ? 'Sadece Yarın İçin' : 'Only for Tomorrow'),
                    ),
                    TextButton(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      color: alarm.isActive ? theme.textTheme.bodyLarge?.color : theme.disabledColor,
                    ),
                  ),
                  Row(
                      children: [
                          if (alarm.title.isNotEmpty && alarm.title != "Alarm")
                             Container(
                                 ...
                                 child: Text(
                                     alarm.title,
                                     style: TextStyle(fontSize: 13, color: theme.textTheme.bodyMedium?.color?.withAlpha(150)),
                                 )
                             ),
...
class _AddEditAlarmSheetState extends State<_AddEditAlarmSheet> {
                ...
                Padding(
                    ...
                    child: Row(
                        ...
                        children: [
                            ...
                            Text(widget.alarm == null ? l10n.newAlarm : l10n.editAlarm, style: Theme.of(context).textTheme.titleMedium),
                            TextButton(
                                onPressed: _save,
  ...
  Widget _buildDateSection(AppLocalizations l10n) {
      ...
      final isToday = _selectedTime.day == DateTime.now().day && _selectedTime.month == DateTime.now().month;
      final isTomorrow = _selectedTime.day == DateTime.now().add(const Duration(days: 1)).day;
      
      String dateText = DateFormat('EEE, d MMM', locale.toString()).format(_selectedTime);
      ...
      return Container(
          ...
          child: ListTile(
              ...
              trailing: Row(
                   ...
                   children: [
                       Text(dateText, style: TextStyle(color: theme.primaryColor, fontWeight: FontWeight.bold)),
  ...
  Widget _buildRepeatSection(AppLocalizations l10n) {
      ...
      final firstDayIndex = MaterialLocalizations.of(context).firstDayOfWeekIndex; // 0=Sunday, 1=Monday
      ...
      int startDay = firstDayIndex == 0 ? 7 : firstDayIndex; // Convert 0(Sun) to 7(Sun) for DateTime
              ...
              children: [
                   ListTile(
                       ...
                       trailing: Row(
                           mainAxisSize: MainAxisSize.min,
                           children: [
                               Text(_getRepeatSummary(l10n), style: TextStyle(color: theme.disabledColor, fontSize: 14)),
                           ...
                           child: Column(
                               children: [
                                   Padding(
                                     ...
                                     child: Row(
                                       ...
                                       children: orderedDays.map((day) {
                                           ...
                                           String label = DateFormat.E(locale).format(date);
                                   ...
                                   Row(
                                       mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                       children: [
                                           ActionChip(label: Text(l10n.weekdays ?? "Weekdays"), onPressed: () => setState(() => _repeatDays = [1,2,3,4,5])),
                                           ActionChip(label: Text(l10n.weekends ?? "Weekends"), onPressed: () => setState(() => _repeatDays = [6,7])),
                                       ]
                                   )
                               ]
                           ),
  ...
  Widget _buildDayToggle(int dayIndex, String label) {
      ...
      return GestureDetector(
          ...
          child: Container(
              width: 36, height: 36, // Slightly larger
  ...
  String _getRepeatSummary(AppLocalizations l10n) {
      ...
      if (_repeatDays.length == 5 && [1,2,3,4,5].every((d) => _repeatDays.contains(d))) return "Weekdays"; // todo localize
      if (_repeatDays.length == 2 && [6,7].every((d) => _repeatDays.contains(d))) return "Weekends";
