...
class ActionService {
  ...
  Future<String> _createAlarm(
    ...
    return l10n.alarmSet(
      time.formatted,
    ); // Assuming localize method exists or constructing string
  }
  ...
  Future<String> _createReminder(
    ...
    if (relativeDate != null) {
      scheduledDate = scheduledDate.add(
        Duration(days: relativeDate.daysFromNow),
      );
    }
  ...
  Future<String> _listNotes(bool isTurkish) async {
    ...
    for (var i = 0; i < notes.length && i < 10; i++) {
      final n = notes[i];
      final preview = n.content.length > 30
  ...
  Future<String> _listReminders(bool isTurkish) async {
    ...
    final buffer = StringBuffer();
    buffer.writeln(
      isTurkish ? '🔔 **Hatırlatıcılarınız:**' : '🔔 **Your Reminders:**',
    );
    ...
    if (upcoming.length > 10) {
      buffer.writeln(
        "... ${isTurkish ? 've ${upcoming.length - 10} hatırlatıcı daha' : 'and ${upcoming.length - 10} more'}",
      );
    }
