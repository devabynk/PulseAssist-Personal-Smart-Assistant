...
class DataService {
  ...
  static Future<void> exportData(BuildContext context, bool isTurkish) async {
    try {
          ...
          if (Hive.isBoxOpen(boxName)) {
            ...
            if (boxPath != null) {
              ...
              if (await boxFile.exists()) {
                await encoder.addFile(boxFile, '$boxName.hive');
              }
            }
          }
        } catch (e) {
      ...
      await encoder.addFile(tempPrefsFile, _prefsFileName);
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isTurkish ? 'Dışa aktarma başarısız: $e' : 'Export failed: $e',
            ),
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (!hasPrefs && !hasHiveData) {
          throw Exception(
            isTurkish ? 'Geçersiz yedek dosyası' : 'Invalid backup file',
          );
        }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isTurkish ? 'İçe aktarma başarısız: $e' : 'Import failed: $e',
            ),
            backgroundColor: Colors.red,
          ),
        );
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isTurkish ? 'Sıfırlama başarısız: $e' : 'Reset failed: $e',
            ),
            backgroundColor: Colors.red,
          ),
        );
