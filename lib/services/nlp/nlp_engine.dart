...
class NlpEngine {
  ...
  String _generateResponse(
    ...
    switch (intent.type) {
      case IntentType.greeting:
        var greeting = _randomResponse(
          isTurkish ? ResponsesTr.greeting : ResponsesEn.greeting,
        );
      ...
      case IntentType.farewell:
        return _randomResponse(
          isTurkish ? ResponsesTr.farewell : ResponsesEn.farewell,
        );

      case IntentType.thanks:
        return _randomResponse(
          isTurkish ? ResponsesTr.thanks : ResponsesEn.thanks,
        );
      ...
      case IntentType.about:
        return _randomResponse(
          isTurkish ? ResponsesTr.about : ResponsesEn.about,
        );
      ...
      case IntentType.compliment:
        return _randomResponse(
          isTurkish ? ResponsesTr.compliment : ResponsesEn.compliment,
        );
      ...
      case IntentType.affirmative:
        final resp = _randomResponse(
          isTurkish ? ResponsesTr.affirmative : ResponsesEn.affirmative,
        );
        return userName != null
            ? '$resp ${isTurkish ? userName : userName}'
            : resp; // Simple append

      case IntentType.negative:
        final resp = _randomResponse(
          isTurkish ? ResponsesTr.negative : ResponsesEn.negative,
        );
        return userName != null
            ? '$resp ${isTurkish ? userName : userName}'
            : resp;
  ...
  String _generateSetNameResponse(
    Map<String, dynamic> entities,
    bool isTurkish,
  ) {
    ...
    return _randomResponse(
      isTurkish ? ResponsesTr.unclear : ResponsesEn.unclear,
    );
  }
  ...
  String _generateTimeResponse(bool isTurkish) {
    ...
    final templates = isTurkish
        ? ResponsesTr.timeTemplates
        : ResponsesEn.timeTemplates;
  ...
  String _generateDateResponse(bool isTurkish) {
    ...
    final weekday = isTurkish
        ? weekdaysTr[now.weekday]
        : weekdaysEn[now.weekday];
    final date = isTurkish
        ? '${now.day} ${monthsTr[now.month]} ${now.year}'
        : '${monthsEn[now.month]} ${now.day}, ${now.year}';

    final templates = isTurkish
        ? ResponsesTr.dateTemplates
        : ResponsesEn.dateTemplates;
    return _randomResponse(
      templates,
    ).replaceAll('{weekday}', weekday).replaceAll('{date}', date);
  }
  ...
  String _generateAlarmResponse(Map<String, dynamic> entities, bool isTurkish) {
    ...
    if (time != null) {
      return isTurkish
          ? 'Alarmı ${time.formatted} için ayarlıyorum.'
          : 'Setting alarm for ${time.formatted}.';
    }
    return isTurkish ? 'Saat kaç?' : 'What time?';
  }

  String _generateReminderResponse(
    Map<String, dynamic> entities,
    bool isTurkish,
  ) {
    ...
    return content != null
        ? (isTurkish
              ? "'$content' hatırlatıcısı oluşturuyorum."
              : "Creating reminder: '$content'.")
  ...
  String _generateHoroscopeResponse(bool isTurkish) {
    ...
    if (input.contains('bugün') ||
        input.contains('bugun') ||
        input.contains('today') ||
        input.contains('nasıl') ||
        input.contains('nasil')) {
  ...
  String _generateBudgetResponse(bool isTurkish) {
    ...
    if (input.contains('tasarruf') ||
        input.contains('saving') ||
        input.contains('biriktir')) {
      return _randomResponse(responses['saving']!);
    } else if (input.contains('takip') ||
        input.contains('track') ||
        input.contains('fatura') ||
        input.contains('bill')) {
  ...
  String _generateEmotionalResponse(bool isTurkish) {
    ...
    if (input.contains('üzgün') ||
        input.contains('uzgun') ||
        input.contains('sad') ||
        input.contains('mutsuz')) {
      return _randomResponse(responses['sad']!);
    } else if (input.contains('mutlu') ||
        input.contains('happy') ||
        input.contains('sevinç') ||
        input.contains('sevincli')) {
...
class NlpResponse {
  ...
  @override
  String toString() =>
      'NlpResponse(intent: ${intent.type}, entities: $entities)';
}
