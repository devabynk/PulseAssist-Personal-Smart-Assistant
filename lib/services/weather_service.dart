...
class WeatherService {
  ...
  Future<Map<String, dynamic>?> getCoordinates(
    ...
    try {
      ...
      return await _keyManager.executeWithRetry((apiKey) async {
        final url = Uri.parse(
          '$_baseUrl/geo/1.0/direct?q=$encodedQuery&limit=1&appid=$apiKey',
        );
  ...
  Future<Map<String, String>?> _reverseGeocode(
    ...
    try {
      final url = Uri.parse(
        '$_baseUrl/geo/1.0/reverse?lat=$lat&lon=$lon&limit=1&appid=$apiKey',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isNotEmpty) {
          ...
          return {'name': localizedName, 'state': state, 'country': country};
        }
      } else if (response.statusCode == 401 || response.statusCode == 429) {
        throw Exception('${response.statusCode}');
      }
  ...
  Future<List<Map<String, String>>> searchLocations(
    String query, {
    String? language,
    int limit = 5,
  }) async {
    ...
    try {
      ...
      return await _keyManager.executeWithRetry((apiKey) async {
        final url = Uri.parse(
          '$_baseUrl/geo/1.0/direct?q=$encodedQuery&limit=$limit&appid=$apiKey',
        );
        ...
        if (response.statusCode == 200) {
          ...
          for (var location in data) {
            ...
            final name = location['name'] as String? ?? '';
            var state = location['state']?.toString() ?? '';
            final country = location['country'] as String? ?? '';
            ...
            if (location['local_names'] != null) {
              final localNames =
                  location['local_names'] as Map<String, dynamic>;
            ...
            if (lat != 0.0 && lon != 0.0) {
              try {
                final reverseData = await _reverseGeocode(
                  lat,
                  lon,
                  apiKey,
                  language: lang,
                ...
                if (reverseData != null) {
                  ...
                  if (reverseName.isNotEmpty &&
                      reverseName != localizedName &&
                      reverseName != name) {
            ...
            final parts = <String>[localizedName];
