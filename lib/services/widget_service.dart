...
class WidgetService {
  ...
  static const String _singleReminderWidgetName =
      'SingleReminderWidgetProvider';
    ...
    if (notes.isEmpty) {
      ...
      final notesData = recentNotes.map((note) {
        return {
          ...
          'content': note.content.startsWith('[')
              ? strings['tapToOpen']!
  ...
  static Future<void> updateRemindersListWidget(
    ...
    if (reminders.isEmpty) {
      ...
      final remindersData = upcomingReminders.map((reminder) {
        ...
        final isToday =
            reminder.dateTime.year == now.year &&
  ...
  static Future<void> updateSingleReminderWidget(
    List<Reminder> reminders,
  ) async {
    final strings = _getLocalizedStrings();
    ...
    if (reminders.isEmpty) {
      ...
      if (incompleteReminders.isEmpty) {
        ...
        await HomeWidget.saveWidgetData<String>(
          ...
          isOverdue
              ? strings['overdue']!
              : strings['dueIn']!.replaceAll('{time}', timeUntil),
        );
      }
    }
